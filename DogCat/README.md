...
I start with an Nmap scan and it reveilled two open ports:
* 22 ssh
* 80 http
...
By clicking on the buttons and checking the source code, we can see that there are two directories: `/dogs` for dog pics and `/cats`, for cat pics. Also `gobuster` didn't find anything interesting, exept these two directories.
...
Now that we have the script, we can figure out how it works. It checks if a parameter `ext` exists and if it doesn't, it appends `.php` to the included page which is `view` parameter. We can use this to include a non-php file. It checks if the `dog` or `cat` is a part of the `view` parameter which we bypassed earlier (./dog/../index).

I also read the other two php files (`cat.php` and `dog.php`) with the same method and they simply just choose a random number between 1 and 10 and append `.jpg` to them and return the pic with that name.
...
I use `burpsuite` from now on to make things easier to understand.

<p align="center"><img src="./files/burp1.png"></p>

As we can see, all the requests we've made and the user-agents are here.

Now let's try to change the user-agent and see the results.
...
Now that we can execute commands, we can upload a reverse shell which you can get [here](https://raw.githubusercontent.com/pentestmonkey/php-reverse-shell/master/php-reverse-shell.php) and get a shell. First I started a server and set uo a listener and then sent the following request.
...
I looked more in the directories but couldn't find other flags then I realized something in `/`:
~~~
cd /
ls -la
total 80
...
I found a file called `backup.sh` in `/opt/backup` and by running the `ls -l`, we can see that the tar file has been modified recently which means that the bash script is running every minute and crating a new tar file.

We can use it to escape. I changed the bash script into a bash reverse shell and started a listener on my machine and now we are out.
...
after waiting a few seconds I got a shell as root:
~~~
┌──(user㉿Y0B01)-[~/…/walkthroughs/thm/DogCat/files]
└─$ rlwrap nc -lvnp 8888
